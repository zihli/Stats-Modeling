...
for(i in 1:19){
  ...
  val.errors[i]=mean((Hitters$Salary[-train]-pred)^2)# MSE
}
...
plot(fit.lasso,xvar="dev",label=TRUE)# x will be R^2 equivalent to deviance
cv.lasso=cv.glmnet(x,y)# one standard error rule
plot(cv.lasso)
coef(cv.lasso)
```
